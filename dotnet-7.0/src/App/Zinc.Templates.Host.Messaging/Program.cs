    ...
    internal static class Program
    {
        ...
        public static async Task<int> Main(string[] args)
        {
            ...
            catch (Exception e)
            {
                Log.Fatal(
                    e,
                    "[MESSAGING]==> Failed to initialize {Host}.\n[MESSAGING]<== ERROR 500: {Message}\n{Stack}",
                    ApplicationHostName,
                    e.Message,
                    e.StackTrace);
            ...
            catch (Exception e)
            {
                Log.Fatal(
                    e,
                    "[MESSAGING]==> A fatal {Error} occurred in {Host}.\n[MESSAGING]<== ERROR 500: {Message}\n{Stack}",
                    e.GetType().Name,
                    ApplicationHostName,
                    e.Message,
                    e.StackTrace);
        ...
        private static IHostBuilder CreateHostBuilder(string[] args) =>
            Microsoft.Extensions.Hosting.Host.CreateDefaultBuilder(args)
                ...
                .ConfigureNServiceBus(ApplicationContext.ApplicationName, EndpointType.FullDuplex)
